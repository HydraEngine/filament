...
Shader::Shader(DriverApi& api, Cleanup& cleanup, ShaderConfig config) : mCleanup(cleanup) {
    ...
    for (unsigned char i = 0; i < config.uniforms.size(); ++i) {
        kLayouts[i] =
                { config.uniforms[i].type.value_or(DescriptorType::UNIFORM_BUFFER), ShaderStageFlags::ALL_SHADER_STAGE_FLAGS, i };
    };

    filamat::DescriptorSets descriptors;
    descriptors[1] = filamat::DescriptorSetInfo(config.uniforms.size());
    for (unsigned char i = 0; i < config.uniforms.size(); ++i) {
        descriptors[1][i] = { config.uniforms[i].name, kLayouts[i], config.uniforms[i].samplerInfo };
    }
    ...
    for (unsigned char i = 0; i < config.uniforms.size(); ++i) {
        bindingsInfo[i] = { config.uniforms[i].name, config.uniforms[i].type.value_or(DescriptorType::UNIFORM_BUFFER), i };
    }
